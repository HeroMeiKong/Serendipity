...
1. ℹ️行内元素有哪些？块级元素有哪些？空(`void`)元素有那些？
...
1. ℹ️`HTML5` 的离线储存怎么使用，它的工作原理是什么？
...
1. ℹ️浏览器是如何对 `HTML5` 的离线储存资源进行管理和加载？
...
1. ℹ️`Canvas` 和 `SVG` 的区别
    ...
    - `SVG`：可缩放矢量图形（`Scalable Vector Graphics`）是基于可扩展标记语言 `XML` 描述的 `2D` 图形的语言，`SVG` 基于 `XML` 就意味着 `SVG DOM` 中的每个元素都是可用的，可以为某个元素附加 `Javascript` 事件处理器。在 `SVG` 中，每个被绘制的图形均被视为对象。如果 `SVG` 对象的属性发生变化，那么浏览器能够自动重现图形。
      ...
      - 不适合游戏应用
    - `Canvas`：画布，通过 `Javascript` 来绘制 `2D` 图形，是逐像素进行渲染的。其位置发生改变，就会重新进行绘制。
...
1. ℹ️文档声明（`Doctype`）和 `<!DOCTYPE HTML>` 有何作用? 严格模式与混杂模式如何区分？它们有何意义?
